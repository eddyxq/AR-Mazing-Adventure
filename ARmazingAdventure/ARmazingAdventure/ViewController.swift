...
//        setupFog()
    ...
    @IBAction func handleSceneTap(_ sender: UITapGestureRecognizer) {
        ...
        guard let hitTestResult = ARCanvas
            .hitTest(sender.location(in: ARCanvas), types: [.existingPlaneUsingGeometry, .estimatedHorizontalPlane])
            .first
            else { return }
    ...
    @IBAction func shareSession(_ button: UIButton) {
        ARCanvas.session.getCurrentWorldMap { worldMap, error in
    ...
    func loadMaze() -> SCNNode{
        return ARCanvas.scene.rootNode
    }
}
